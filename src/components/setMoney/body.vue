<template>
    <div class="lln">
        ...
        <p class="zmy">转入金额(元)</p>
        <div class="mon">
            <span>￥</span>
            <input ref="ipt" @input="del" v-model="money" placeholder="请输入转入金额" type="Number">
            <span ref="all" @click="all">全部</span>
            <span ref="z" class="warn">余额可用额度不足</span>
            <span @click="mov" ref="d" class="cle">x</span>
        </div>
        <p class="pln">预计收益到账时间 <span>{{m}}月{{d}}日</span></p>
        <button ref="b" class="qbtn" @click="makesureInMoney">确认转入</button>
        <p class="mcos">余额转入的资金若需转出至银行卡，请提前转出至余额后提现，提现将收取提现服务费，请对资金进行合理安排</p>
...
<script>
import setMoney from '../../apis/setmoneyApi.js'
    ...
    methods:{
        ...
        makesureInMoney(){
            if(this.$refs.b.style.color == "white"){
                ...
                setMoney.getsetmoneyInfo(user,this.money,(data)=>{
